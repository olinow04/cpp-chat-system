...
public:
    ...
    RabbitMQConsumer(const std::string& host, int port,
        ...
        try {
            ...
            constexpr int DEFAULT_CHANNEL_MAX = 0;
            constexpr int DEFAULT_FRAME_MAX = 131072;  // 128KB max frame size
            constexpr int DEFAULT_HEARTBEAT = 0;       // Disabled

            amqp_rpc_reply_t reply = amqp_login(
                conn_,
                "/",                      // vhost (default virtual host)
                DEFAULT_CHANNEL_MAX,      // channel_max (0 = no limit)
                DEFAULT_FRAME_MAX,        // frame_max (128KB)
                DEFAULT_HEARTBEAT,        // heartbeat (0 = disabled)
                AMQP_SASL_METHOD_PLAIN,   // auth method
                user.c_str(),
                password.c_str()
            );
            ...
            // Declare queue
            std::cout << "Declaring queue '" << queueName_ << "'..." << std::endl;
            amqp_queue_declare(
                conn_,
                CHANNEL_ID,
    ...
    void startConsuming() {
        ...
        constexpr int TIMEOUT_MICROSECONDS = 0;
...
private:
    ...
    void simulateEmailDelay() {
        constexpr int DELAY_MILLISECONDS = 1500;
